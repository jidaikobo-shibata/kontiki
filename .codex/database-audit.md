...
- metadata名で並べ替えても `meta_data` をjoinせず、SQLiteではエラーにならないまま
  並べ替えが黙って無視される
...
7. metadataを含む検索・並べ替えが黙って機能しない現状のテスト
...
許可を得て、実運用環境でも同じ読み取り専用集計を行った。ホスト名、パス、正確な
件数は公開リポジトリに含めず、非公開の運用摘録へ記録した。

- 9件のマイグレーションが適用済みで、`integrity_check` は `ok`
- サイト側に独自modelや有効な独自route登録はない
- コンテンツは `post` typeだけを利用している
- publishedとpendingを利用している
- `terms`、`term_relationships`、`meta_data` は空
- 数値として有効な記事の親子関係と、初期値以外の `sort_order` はない
- post、term relationship、metadataについて、確認対象となる孤児参照はない
- post type、slug、statusに空値または未知の値はない
- 全記事で `updated_at` と `created_at` が同値だった
- `parent_id` は全記事でSQLiteのTEXTとして保存されていた
- SQLiteの外部キー検査は無効

これにより、taxonomyや現在のmetadata実装をv1の初期互換経路として即座に動かす
必要性は低いと判断できる。ただし、公開APIとして存在するコードを削除する判断とは
分ける。まず未完成状態を特性テストで固定し、非推奨化または再設計を明示する。
