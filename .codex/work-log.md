...
- 安全な公開版 `v0.9.64` から `0.9-maintenance` ブランチを作成し、originへ公開。
- `0.9-maintenance` は既存サイト向けのセキュリティ修正・重大バグ修正に限定する。
- `main` はv1開発に使用し、破壊的変更や統合作業をv0.9へ混入させない。
- v0.9タグは既存サイトで受け入れ可能と検証できた修正にだけ付ける。
